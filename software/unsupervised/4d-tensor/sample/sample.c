...
#include <string.h>
#include <math.h>
// base feature set for 4d-tensor
...
// csf features set for 4d-tensor
...
// 4d tensor flatten mapping
int* getFlattenInput(char* tns_dir, int output_resolution) {
    int* imgs = (int*)malloc(sizeof(int) * 4 * output_resolution * output_resolution);
    int order, i, j, k;
    FILE* fpt;
    fpt = fopen(tns_dir, "r");
    if (fpt == NULL)
    ...
    fscanf(fpt, "%d %d %d %d", &dim[0], &dim[1], &dim[2], &dim[3]);
    // get the dims of four modes
    int* dim_mode = (int*)malloc(sizeof(int) * 4 * 2);
    float* scaledim = (float*)malloc(sizeof(float) * 4 * 2);
    int* mode_valdim = (int*)malloc(sizeof(int) * 4 * 2);
    int* indexdim = (int*)malloc(sizeof(int) * 4 * 2);
    int* valdim = (int*)malloc(sizeof(int) * 4);
    float value;
    dim_mode[0] = dim[0]; dim_mode[1] = dim[1] * dim[2] * dim[3];
    dim_mode[2] = dim[1]; dim_mode[3] = dim[0] * dim[2] * dim[3];
    dim_mode[4] = dim[2]; dim_mode[5] = dim[0] * dim[1] * dim[3];
    dim_mode[6] = dim[3]; dim_mode[7] = dim[0] * dim[1] * dim[2];
    // get the scaldim
    for (i = 0; i < 4; i++)
        for (j = 0; j < 2; j++)
            scaledim[i*2+j] = dim_mode[i*2+j] / (output_resolution * 1.0);
    // init imgs
    for (i = 0; i < 4 * output_resolution * output_resolution; i++)
        imgs[i] = 0;
    //get the density sampling of four images
    while (fscanf(fpt, "%d %d %d %d %f\n", &valdim[0], &valdim[1], &valdim[2], &valdim[3], &value) != EOF) {
        mode_valdim[0] = valdim[0]-1; mode_valdim[1] = (valdim[3]-1)*dim[2]*dim[1]+(valdim[2]-1)*dim[1]+valdim[1]-1;
        mode_valdim[2] = valdim[1]-1; mode_valdim[3] = (valdim[3]-1)*dim[2]*dim[0]+(valdim[2]-1)*dim[0]+valdim[0]-1;
        mode_valdim[4] = valdim[2]-1; mode_valdim[5] = (valdim[3]-1)*dim[1]*dim[0]+(valdim[1]-1)*dim[0]+valdim[0]-1;
        mode_valdim[6] = valdim[3]-1; mode_valdim[7] = (valdim[2]-1)*dim[1]*dim[0]+(valdim[1]-1)*dim[0]+valdim[0]-1;
        for (i = 0; i < 4; i++)
            for (j = 0; j < 2; j++)
...
// 4d tensor map sampling
int* getMapInput(char* tns_dir, int output_resolution) {
    int* imgs = (int*)malloc(sizeof(int) * 6 * output_resolution * output_resolution);
    int order, i, j, k;
    FILE* fpt;
    fpt = fopen(tns_dir, "r");
    if (fpt == NULL)
    ...
    int* dim = (int*)malloc(sizeof(int) * order);
    fscanf(fpt, "%d %d %d %d", &dim[0], &dim[1], &dim[2], &dim[3]);
    // density sampling of four images
    ...
    // init imgs
    for (i = 0; i < 6 * output_resolution * output_resolution; i++)
        imgs[i] = 0;
    while (fscanf(fpt, "%d %d %d %d %f\n", &valdim[0], &valdim[1], &valdim[2], &valdim[3], &value) != EOF) {
        for (i = 0; i < 4; i++)
            indexdim[i] = (int)((valdim[i]-1) / scaledim[i]);
        imgs[indexdim[0]*output_resolution+indexdim[1]] += 1;
        imgs[output_resolution*(1*output_resolution+indexdim[0])+indexdim[2]] += 1;
        imgs[output_resolution*(2*output_resolution+indexdim[0])+indexdim[3]] += 1;
        imgs[output_resolution*(3*output_resolution+indexdim[1])+indexdim[2]] += 1;
        imgs[output_resolution*(4*output_resolution+indexdim[1])+indexdim[3]] += 1;
        imgs[output_resolution*(5*output_resolution+indexdim[2])+indexdim[3]] += 1;
    }
    fclose(fpt);
...
float* getCsfFeatures(char* tns_dir) {
    ...
    // get dims
    fscanf(fpt, "%d %d %d %d", &ftset.dim[0], &ftset.dim[1], &ftset.dim[2], &ftset.dim[3]);
    int* valdim = (int*)malloc(sizeof(int) * ftset.order);
    int* lastdim = (int*)malloc(sizeof(int) * ftset.order);
    float value;
    fscanf(fpt, "%d %d %d %d %f\n", &lastdim[0], &lastdim[1], &lastdim[2], &lastdim[3], &value);
    int  nnz = 1, numSlices = 1, numFibers = 1;
    while (fscanf(fpt, "%d %d %d %d %f\n", &valdim[0], &valdim[1], &valdim[2], &valdim[3], &value) != EOF) {
        if (!(valdim[0] == lastdim[0] && valdim[1] == lastdim[1])) {
            numSlices += 1;
            numFibers += 1;
        }
        else {
            if (valdim[2] != lastdim[2])
                numFibers += 1;
        }
        lastdim[0] = valdim[0];
        lastdim[1] = valdim[1];
    ...
    fclose(fpt);
    ftset.nnz = nnz;
    ftset.density = ftset.nnz / (ftset.dim[0] * ftset.dim[1] * ftset.dim[2] *ftset.dim[3] * 1.0);
    ...
    // get the features
    float* features = (float*)malloc(sizeof(float) * 13);
    features[0] = ftset.numSlices;
    features[1] = ftset.numFibers;
    features[2] = ftset.sliceRatio;
    features[3] = ftset.fiberRatio;
    features[4] = ftset.maxFibersPerSlice;
    features[5] = ftset.minFibersPerSlice;
    features[6] = ftset.aveFibersPerSlice;
    features[7] = ftset.maxNnzPerSlice;
    features[8] = ftset.minNnzPerSlice;
    features[9] = ftset.aveNnzPerSlice;
    features[10] = ftset.maxNnzPerFiber;
    features[11] = ftset.minNnzPerFiber;
    features[12] = ftset.aveNnzPerFiber;
...
float* getBaseFeatures(char* tns_dir) {
    ...
    int* dim3_nnz = (int*)malloc(sizeof(int) * ftset.dim[3]);
    ...
    for (i = 0; i < ftset.dim[2]; i++)
        dim2_nnz[i] = 0;
    for (i = 0; i < ftset.dim[3]; i++)
        dim3_nnz[i] = 0;
    while (fscanf(fpt, "%d %d %d %d %f\n", &valdim[0], &valdim[1], &valdim[2], &valdim[3], &value) != EOF) {
        dim0_nnz[valdim[0]-1] += 1;
        dim1_nnz[valdim[1]-1] += 1;
        dim2_nnz[valdim[2]-1] += 1;
        dim3_nnz[valdim[3]-1] += 1;
        nnz += 1;
    }
    fclose(fpt);

    ftset.nnz = nnz;
    ftset.density = (nnz * 1.0) / (ftset.dim[0] * ftset.dim[1] * ftset.dim[2] * ftset.dim[3]);
    // initialize max_nnz, min_nnz, tot_nnz, etc
    ftset.max_nnz[0]=dim0_nnz[0]; ftset.max_nnz[1]=dim1_nnz[0]; 
    ftset.max_nnz[2]=dim2_nnz[0]; ftset.max_nnz[3]=dim3_nnz[0];
    ftset.min_nnz[0]=dim0_nnz[0]; ftset.min_nnz[1]=dim1_nnz[0]; 
    ftset.min_nnz[2]=dim2_nnz[0]; ftset.min_nnz[3]=dim3_nnz[0];
    ftset.ave_nnz[0]=dim0_nnz[0]; ftset.ave_nnz[1]=dim1_nnz[0]; 
    ftset.ave_nnz[2]=dim2_nnz[0]; ftset.ave_nnz[3]=dim3_nnz[0];
    ftset.dev_nnz[0]=dim0_nnz[0]; ftset.dev_nnz[1]=dim1_nnz[0]; 
    ftset.dev_nnz[2]=dim2_nnz[0]; ftset.dev_nnz[3]=dim3_nnz[0];
    ftset.bounce[0]=dim0_nnz[0]; ftset.bounce[1]=dim1_nnz[0]; 
    ftset.bounce[2]=dim2_nnz[0]; ftset.bounce[3]=dim3_nnz[0];
    ...
    tot_nnz[0] = dim0_nnz[0]; tot_nnz[1] = dim1_nnz[0]; 
    tot_nnz[2] = dim2_nnz[0]; tot_nnz[3] = dim3_nnz[0];
    dim_counter[0]=compare(dim0_nnz[0],0); dim_counter[1]=compare(dim1_nnz[0],0); 
    dim_counter[2]=compare(dim2_nnz[0],0); dim_counter[3]=compare(dim3_nnz[0],0);
    adj_nnz[0]=0.0; adj_nnz[1]=0.0; adj_nnz[2]=0.0; adj_nnz[3]=0.0;
    ...
    for (i = 1; i < ftset.dim[3]; i++) {
        if (dim3_nnz[i] > ftset.max_nnz[3])
            ftset.max_nnz[3] = dim3_nnz[i];
        if (dim3_nnz[i] < ftset.min_nnz[3])
            ftset.min_nnz[3] = dim3_nnz[i];
        adj_nnz[3] += abs(dim3_nnz[i] - dim3_nnz[i-1]);
        tot_nnz[3] += dim3_nnz[i];
        dim_counter[3] += compare(dim3_nnz[i], 0);
    }
    ...
    features[4] = ftset.nnz; features[5] = ftset.density;
    for (i = 0; i < 4; i++) {
        features[i + 6] = ftset.max_nnz[i];
    }
    for (i = 0; i < 4; i++) {
        features[i + 10] = ftset.min_nnz[i];
    }
    for (i = 0; i < 4; i++) {
        features[i + 14] = ftset.dev_nnz[i];
    }
    for (i = 0; i < 4; i++) {
        features[i + 18] = ftset.ave_nnz[i];
    }
    for (i = 0; i < 4; i++) {
        features[i + 22] = ftset.bounce[i];
    }
