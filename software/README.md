...
Note that we conducted the safety checks on network inference. Specifically, we repeated the process of network inference 10 times, and collected the means, medians and percentiles (i.e., 20%, 40%, 60% and 80%) of prediction accuracies. The scripts for collecting data and safety checks are detailed in the `scripts` directory.
