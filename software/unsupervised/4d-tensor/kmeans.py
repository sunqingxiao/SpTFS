...
def main():
    if len(sys.argv) < 4:
        print("usage: {} {train data} {test data} {result data}")
    ...
    ## map images
    map_encoded_imgs = []
    autoencoder = ConvAutoencoder(num_map_output)
    autoencoder.compile(optimizer='adam', loss=losses.MeanSquaredError())
    autoencoder.fit(trainset.images[:,1,:,:,tf.newaxis], trainset.images[:,1,:,:,tf.newaxis],
                    epochs=20,
                    batch_size=100,
                    shuffle=True)
    map_encoded_imgs.append(autoencoder.encoder(testset.images[:,1,:,:,tf.newaxis]).numpy())

    autoencoder.compile(optimizer='adam', loss=losses.MeanSquaredError())
    autoencoder.fit(trainset.images[:,2,:,:,tf.newaxis], trainset.images[:,2,:,:,tf.newaxis],
                    epochs=20,
                    batch_size=100,
                    shuffle=True)
    map_encoded_imgs.append(autoencoder.encoder(testset.images[:,2,:,:,tf.newaxis]).numpy())

    autoencoder.compile(optimizer='adam', loss=losses.MeanSquaredError())
    autoencoder.fit(trainset.images[:,3,:,:,tf.newaxis], trainset.images[:,3,:,:,tf.newaxis],
                    epochs=20,
                    batch_size=100,
                    shuffle=True)
    map_encoded_imgs.append(autoencoder.encoder(testset.images[:,3,:,:,tf.newaxis]).numpy())

    encoded_imgs = np.concatenate((testset.features, flatten_encoded_imgs, map_encoded_imgs[0], map_encoded_imgs[1], map_encoded_imgs[2], testset.features), axis=1)
